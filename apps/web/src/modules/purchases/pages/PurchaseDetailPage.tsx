...
export function PurchaseDetailPage() {
  ...
  if (isError || !purchase)
    return <div className="p-8 text-center text-sm text-red-500">Purchase not found</div>;

  const handleCancel = async () => {
    await cancelMutation.mutateAsync(purchase.id);
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3">
            <h2 className="text-xl font-bold text-gray-900">{purchase.purchaseNumber}</h2>
      ...
      {isCancelled && (
        <div className="bg-gray-100 border border-gray-300 rounded-lg px-4 py-3 text-sm text-gray-600">
          This purchase has been cancelled. Stock and ledger effects have been reversed.
        </div>
      )}

      {/* Info grid */}
      <div className="bg-white rounded-lg border p-5 grid grid-cols-2 gap-4 text-sm">
        <InfoRow label="Party" value={purchase.party?.name ?? purchase.partyId} />
        <InfoRow label="Warehouse" value={purchase.warehouse?.name ?? purchase.warehouseId} />
        <InfoRow
          label="Purchase Date"
          value={new Date(purchase.purchaseDate).toLocaleDateString()}
        />
        <InfoRow label="Vehicle" value={purchase.vehicle?.vehicleNo ?? '—'} />
      </div>
      ...
      <div className="bg-white rounded-lg border overflow-hidden">
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b">
            <tr>
              {['Item', 'Gross Wt', 'Cut Wt', 'Net Wt', 'Rate', 'Amount'].map((h) => (
          ...
          <tbody className="divide-y divide-gray-100">
            {(purchase.items ?? []).map((item) => (
              <tr key={item.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 tabular-nums">{Number(item.grossWeight).toFixed(3)}</td>
                <td className="px-4 py-3 tabular-nums">{Number(item.cutWeight).toFixed(3)}</td>
                <td className="px-4 py-3 tabular-nums font-medium">{Number(item.netWeight).toFixed(3)}</td>
                <td className="px-4 py-3 tabular-nums">{Number(item.rate).toFixed(2)}</td>
                <td className="px-4 py-3 tabular-nums font-medium">{Number(item.amount).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="bg-gray-50 rounded-lg border p-5">
        <div className="flex flex-col items-end gap-1 text-sm">
          <TotalRow label="Gross Amount" value={Number(purchase.grossAmount)} />
          <TotalRow label="Expense Amount" value={Number(purchase.expenseAmount)} />
          <div className="border-t w-64 pt-2 mt-1">
            <TotalRow label="Net Amount" value={Number(purchase.netAmount)} bold />
          </div>
        </div>
      </div>
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-6 space-y-4">
            <h3 className="font-semibold text-gray-900">Cancel this purchase?</h3>
            <p className="text-sm text-gray-600">
              This will reverse the stock and ledger effects of this purchase. The record will
              remain in the system as cancelled. This cannot be undone.
            </p>
            <div className="flex justify-end gap-3 pt-2">
              ...
              >
                {cancelMutation.isPending ? 'Cancelling…' : 'Yes, Cancel Purchase'}
              </button>
            </div>
          </div>
        </div>
